          ...
          {stats.slice(0, 3).map((stat, index) => (
            <div key={stat.label} className="contents">
              <div className="text-center">
                <p className="text-2xl font-headline text-stone-400">{stat.value}</p>
                <p className="mt-1 text-xs uppercase tracking-widest">{stat.label}</p>
              </div>
              {index < Math.min(stats.length, 3) - 1 ? (
                <div className="h-12 w-px self-center bg-stone-800" />
              ) : null}
            </div>
          ))}
        </AnimatedSection>
