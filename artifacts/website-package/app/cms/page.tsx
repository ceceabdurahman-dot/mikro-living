...
export default function CMSDashboard() {
  ...
  const loadServiceList = async (authToken = token) => {
    ...
    await runManagedListLoad('services', async () => {
      const payload = await adminFetch<PaginatedResponse<ServiceItem>>(authToken, `/services?${serviceQuery}`)
  ...
  const loadProjectList = async (authToken = token) => {
    ...
    await runManagedListLoad('projects', async () => {
      const payload = await adminFetch<PaginatedResponse<ProjectItem>>(authToken, `/projects?${projectQuery}`)
  ...
  const loadBlogList = async (authToken = token) => {
    ...
    await runManagedListLoad('blog', async () => {
      const payload = await adminFetch<PaginatedResponse<BlogItem>>(authToken, `/blog?${blogQuery}`)
  ...
  const loadTeamList = async (authToken = token) => {
    ...
    await runManagedListLoad('team', async () => {
      const payload = await adminFetch<PaginatedResponse<TeamItem>>(authToken, `/team?${teamQuery}`)
  ...
  const loadLeadList = async (authToken = token) => {
    ...
    await runManagedListLoad('leads', async () => {
      ...
      setLeadDrafts(
        Object.fromEntries(
          payload.data.map((lead) => [lead.id, { status: lead.status, notes: lead.notes || '' }])
        )
      )
  ...
  const hydrate = async (authToken = token, background = false, reloadManagedLists = true) => {
    ...
    try {
      ...
      const [
        ...
        adminFetch<{ data: { role?: string } }>(authToken, '/auth/me'),
      ])
  ...
  const selectAllAcrossDataset = async (
    ...
    try {
      ...
      if (section === 'projects') {
        ...
        const ids = await fetchAllIdsFromEndpoint<ProjectItem>(`/projects${query ? `?${query}` : ''}`)
      ...
      if (section === 'blog') {
        ...
        const ids = await fetchAllIdsFromEndpoint<BlogItem>(`/blog${query ? `?${query}` : ''}`)
      ...
      const ids = await fetchAllIdsFromEndpoint<TeamItem>(`/team${query ? `?${query}` : ''}`)
  ...
  return (
    <div className="flex min-h-screen bg-stone-50 font-body">
      ...
      {activeSelectionCount > 0 ? (
        <div className="fixed bottom-5 left-1/2 z-[65] flex w-[min(920px,calc(100%-2rem))] -translate-x-1/2 flex-wrap items-center justify-between gap-4 rounded-[24px] border border-stone-200 bg-white/95 px-5 py-4 shadow-xl backdrop-blur">
          <div>
                ...
                >
                  <div className="grid gap-5 xl:grid-cols-[1fr_0.92fr]">
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    setBlogForm({
                                      ...
                                      read_time: item.read_time?.toString() || '5',
                                      meta_title: item.meta_title || '',
                                      meta_desc: item.meta_desc || '',
                                    })
                                  }}
                    ...
                    <div className="rounded-[22px] border border-stone-200 bg-white p-5 shadow-sm">
                      ...
                      <Field label="Content">
                        <textarea className={`${inputClassName} mt-2 min-h-56`} value={blogForm.content} onChange={(e) => setBlogForm((c) => ({ ...c, content: e.target.value }))} />
                      </Field>
                      <AssetPreview
                        label="Current cover"
                        url={posts.find((post) => post.id === editingPostId)?.cover_url}
                      />
                      <Field label="Cover image">
                        <input
                          type="file"
