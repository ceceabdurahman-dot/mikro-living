...
import { notifyLeadSubmitted } from '../lib/leadSync'
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="space-y-2">
                  <span className="text-sm font-semibold text-stone-700">Budget range</span>
                  <select
                    value={form.budget_range}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, budget_range: event.target.value }))
                    }
                    className="w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-primary focus:bg-white"
                  >
                    <option value="">Select a range</option>
                    <option value="under-50m">Under 50 juta</option>
                    <option value="50-100m">50 - 100 juta</option>
                    <option value="100-250m">100 - 250 juta</option>
                    <option value="250-500m">250 - 500 juta</option>
                    <option value="above-500m">Above 500 juta</option>
                  </select>
                </label>
                <label className="space-y-2 md:col-span-2">
                  <span className="text-sm font-semibold text-stone-700">Area size (sqm)</span>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={form.area_sqm}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, area_sqm: event.target.value }))
                    }
                    className="w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-primary focus:bg-white"
                    placeholder="45"
                  />
